	...
	SubShader{
		...
		Pass{
			...
			v2f vert(appdata v) {
				...
				if (degrees(acos(dot(scaleDir.xyz, v.normal.xyz))) > _Angle) {
					v.vertex.xyz += normalize(v.normal.xyz) * _FirstOutlineWidth * _Power;
				}else {
					v.vertex.xyz += scaleDir * _FirstOutlineWidth * _Power;
				}
		...
		Pass{
			...
			v2f vert(appdata v) {
				...
				//This way prevents weird artifacts from being created when using either of the methods
				if (degrees(acos(dot(scaleDir.xyz, v.normal.xyz))) > _Angle) {
					v.vertex.xyz += normalize(v.normal.xyz) * _SecondOutlineWidth * _Power;
				}
			else {
				v.vertex.xyz += scaleDir * _SecondOutlineWidth * _Power;
			}
